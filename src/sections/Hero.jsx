    ...
    <div className="fixed inset-0 w-full h-full bg-black">
      <AnimatePresence mode="wait">
        {active === null ? (
          ...
          >
            {active === 1 ? (
              <AboutMe onClose={() => setActive(null)} />
            ) : active === 2 ? (
              <Projects onClose={() => setActive(null)} />
            ) : active === 3 ? (
              <SkillsEducation onClose={() => setActive(null)} />
            ) :active === 4 ? (
              <Contact onClose={() => setActive(null)} />
            ) :(
              <div className="flex flex-col items-center justify-center h-full text-white">
                <h2 className="text-4xl font-bold mb-4">
                  {tiles.find((t) => t.id === active)?.title}
                </h2>
                <p className="text-lg">
                  {tiles.find((t) => t.id === active)?.subtitle}
                </p>
                <button
